...
DELIMITER $$
  ...
  DECLARE cur CURSOR FOR SELECT * from checklist where repeteDiariamente = 1;
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1;

  OPEN cur;
  myloop:
  LOOP
    FETCH cur INTO  id, status, titulo, tipoChecklist, descricao, data_limite, dataexecucao, data_cadastro, encerrado, deleted_at, repeteDiariamente, questionador_id, responsavel_id, setor_id, checklist_id, loja_id;
    
            IF done =0 THEN 
              LEAVE myloop;
            END IF;
      IF (repeteDiariamente = 1) THEN;
        insert into checklist(status, titulo, tipoChecklist, descricao, data_limite, repeteDiariamente, questionador_id, responsavel_id, setor_id, checklist_id, loja_id) values ( status,  titulo,  tipoChecklist,  descricao,  NOW(), 0,  questionador_id, responsavel_id, setor_id, checklist_id, loja_id);  
      END IF;
  END LOOP;
  CLOSE cur;
END$$
