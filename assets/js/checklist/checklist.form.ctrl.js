...
(function (window, app, $, toastr) {
	...
	function ControladoraFormChecklist(servicoChecklist, controladoraListagemChecklist) {
		...
		_this.popularColaboradores = function popularColaboradores(valor = 0) {
			...
			var jqXHR = servicoUsuario.todosOpcoes();
			jqXHR.done(sucesso).fail(erro);
		};
		...
		_this.popularQuestionarios = function popularQuestionarios(valor = 0) {
			...
			var jqXHR = servicoQuestionario.todosOpcoes();
			jqXHR.done(sucesso).fail(erro);
		};
		...
		_this.desenhar = function desenhar(resposta) {
			...
			if (window.location.href.search('visualizar') != -1) {
				...
				_this.formulario.find('#botoes').prepend('<div class="col col-md-4 col-12 col-sm-5 col-lg-4"><button type="submit" id="remover" class="waves-effect waves-light btn white grey-text text-darken-4 col-12"><i class="mdi mdi-delete red-text text-darken-4"></i>Remover</button></div>').promise().done(function () {
